...
//             customerAddressPhone($post_data['customerAddressPhone'])->
        ...
        switch($this->gateway->api_payment_modes){
            ...
            case '0':
                ...
                $mmb_form[] = '" method="get"><input type="hidden" name="wcapi" value="boipa"/><input type="hidden" name="order_id" value="'.$order->get_id().'"/><input type="hidden" id="merchantTxId" name="merchantTxId" value=""/></form>';
    ...
    private function get_gateway_form($token,$integration,$sandbox = false){
        ...
        if ($this->automaticForm == 'yes') {
            ...
            $form_html .= '<button type="submit" class="button alt">'.__( 'Pay with BOIPA', 'mmb-gateway-woocommerce' ).'</button>';
        }
        $form_html .= '</form>';
        return $form_html;
    }
